...
- [ ] **Step 2:** aplicar R1 a cada `useEffect` seÃ±alado (cargas iniciales). Recordar que estas pÃ¡ginas usan **staffApi** (`src/api/staffAxios.js`), conservar la ruta del endpoint como este hoy. Si la funcion `cargarX` se reutiliza en handlers (refrescar, tabs), conservarla y duplicar el fetch inline SOLO en el effect.
